...
def build_topology():
    ...
    # Enable RSTP on all switches to prevent broadcast loops
    info('*** Enabling RSTP on all switches\n')
    for sw in net.switches:
        sw.cmd('ovs-vsctl set bridge {} rstp_enable=true'.format(sw.name))

    info('*** Waiting for RSTP convergence (15s)...\n')
    time.sleep(15)
